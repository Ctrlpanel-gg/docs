...
apt install -y php8.3 php8.3-{common,cli,gd,mysql,mbstring,bcmath,xml,fpm,curl,zip} mariadb-server nginx git redis-server
```
...
CtrlPanel uses a few additional dependencies compared to Pterodactyl, so you'll need to install them to ensure everything works properly.
