...
# Updating from 0.9.x to 1.0.1

:::caution
This guide is for updates 0.9.x -> 1.0.1

If you update from 0.8.x, please follow our 0.9.0 update guide which can be found [here](/docs/0.9.0/Installation/updating).
:::
...
```bash title="/etc/nginx/sites-available/ctrlpanel.conf"
...
:::caution Important Step for Updating from Version 0.9.x to 1.0.1

```bash
rm -rf /var/www/ctrlpanel/vendor
```
